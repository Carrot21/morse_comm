...
func readLoop(c WriteCloser, wg *sync.WaitGroup) {
	...
	for {
		...
		case <-cDone: // connection closed
			logger.Debugln("receiving cancel signal from conn %s, release connection", remoteAddr)
			return
		case <-sDone: // server closed
			logger.Debugln("receiving cancel signal from server, release connection")
			return
		default:
			msg, err = codec.Decode(rawConn)
			if err != nil {
				logger.Errorf("Decode message failed. remoteAddr is %s, error is %s", remoteAddr, err.Error())
			...
			if handler == nil {
				if onMessage != nil {
					...
					logger.Warnf("no handler or onMessage() found for message 0x%04x\n", msg.MessageNumber())
				}
				continue
			}
			...
			case pkt = <-sendCh:
				if pkt != nil {
					if _, err = rawConn.Write(pkt); err != nil {
						logger.Errorf("error writing data %s\n", err.Error())
					}
				}
			default:
				break OuterFor
